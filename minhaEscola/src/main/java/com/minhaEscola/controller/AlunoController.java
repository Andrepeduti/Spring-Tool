...
public class AlunoController {
	...
	@PostMapping
	public ResponseEntity<AlunoModel> post (@RequestBody AlunoModel aluno){
		return ResponseEntity.status(HttpStatus.CREATED).body(repository.save(aluno));
	}
	@PutMapping("/{id}")
	public ResponseEntity<AlunoModel> atualiza(@PathVariable Long id, @RequestBody AlunoModel aluno){
		aluno.setId(id);
